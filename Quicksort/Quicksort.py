...
class quicksort:
    ...
    def __init__(self, *args):                  # puede recibir varios argumentos
        ...
        # if len(args)==1:
        #     self.array = args[0]                # el primer argumento es la lista a ordenar
        #     try:
        #         self.pivote = mean(self.array)  # el pivote tiene que estar al medio de los datos
        #         # en principio quise usar la mediana, pero en teoría para hallar la mediana
        #         # ya tengo que tener ordenados los datos
        #     except:                             # el algoritmo solo ordena datos numéricos
        #         print("error con sus datos, verifique que sean todos numéricos")
        #         self.flag_ok = False
        #         return
        # elif len(args) == 2:
        #     self.array = args[0]                # el primer argumento es la lista
        ...
        #         self.flag_ok = False
        #         return
        # else:                                   # solo contemplo introducir 1 o 2 argumentos
        #     self.flag_ok = False
        #     print("Faltan o sobran argumentos")
        #     return
        
        if len(self.array) > 0:     # si la longitud del array es muy corta no es necesario ordenar
            ...
            while self.flag_ok == False:
                ...
                if type(self.array[indice]) != type(0) and type(self.array[indice]) != type(0.0):     # recorro la lista para asegurarme que todos sean numeros
                    ...
                    # indice = self.array.index(i);
                    self.array.remove(self.array[indice])
                else:
                    indice += 1
                
                if indice >= len(self.array):
                    self.flag_ok = True
            ...
            for i in self.array:
                ...
                if type(i) != type(0) and type(i) != type(0.0):     # recorro la lista para asegurarme que todos sean numeros
                    print("Error en datos del array, se eliminará el elemento numero", self.array.index(i), " -> ", i)
                    indice = self.array.index(i);
                    self.array.remove(i)
...
arreglo = ["coco", "pepe", "toto", "lola", "lalo"]
